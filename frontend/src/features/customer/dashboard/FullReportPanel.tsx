...
/** Full "Báo cáo mạng xã hội về MobiFone" content — filters, KPIs, topic
 * tables, sentiment-by-topic chart, negative/positive post lists, and the
 * export/send actions. Self-contained so it can be reused both as the
...
export function FullReportPanel({ actions = 'all' }: { actions?: 'all' | 'word-only' } = {}) {
  ...
  const [exportingEventWord, setExportingEventWord] = useState(false)
  ...
  const topicChartData = data
    ? data.keyword_topic_sentiment.map((row) => ({
        topic: row.topic,
        'Tiêu cực': row.negative,
        'Trung lập': row.neutral,
        'Tích cực': row.positive,
      }))
    : []
      ...
      <Card>
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {actions === 'all' && (
              <>
                <Button variant="outline" size="sm" onClick={handleSendEmailNow} disabled={sendingEmail || isLoading}>
            ...
            {actions === 'all' && (
              <Button variant="outline" size="sm" onClick={handleExportEventWord} disabled={exportingEventWord}>
                <FileText className="h-3.5 w-3.5" />
                {exportingEventWord ? 'Đang xuất…' : 'Xuất báo cáo 5G'}
              </Button>
            )}
          </div>
        </div>
